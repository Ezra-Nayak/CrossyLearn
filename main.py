# --- CrossyLearn: Modern Architecture with Baseline Agent ---
# This version combines the advanced features of Milestone 23 (Producer-Consumer architecture,
# advanced vision, robust game interaction) with the simple, non-learning random agent
# from Milestone 13. It serves as an updated, intelligent baseline without any ML components.
...
# --- AGENT CLASS (FROM MILESTONE 13, ADAPTED FOR MODERN ARCHITECTURE) ---
...
class Agent:
    def __init__(self):
        ...
        self.new_game_cooldown = 2.5 # Cooldown after a new game starts before the first move

    def on_new_game(self):
        """Resets the agent's state for a new game and enforces a cooldown."""
        print("Agent acknowledging new game. Cooldown initiated.")
        self.is_first_move = True
        # Enforce the animation lockout before the first move
        self.last_action_time = time.time() + self.new_game_cooldown
    ...
    def dispatch_click(self, hwnd, click_pos):
        """Sends a mouse click in a separate thread."""
        if hwnd:
            click_thread = threading.Thread(target=send_click, args=(hwnd, click_pos))
            click_thread.start()


# --- VISION & CONTROL FUNCTIONS (FROM MILESTONE 23) ---
...
class VisionProducer(threading.Thread):
    def __init__(self, window_title, output_queue):
        ...
        # Parameters for chicken and penalty zone detection
        ...
        with mss.mss() as sct:
            while self.running:
                try:
                    ...
                    result = {
                        "frame": game_frame, "score": score_val, "is_dead": death_detected,
                    ...
                    try:
                        ...
                        pass # Drop frame if main thread is lagging

                except Exception as e:
                    print(f"Error in VisionProducer: {e}")
                    time.sleep(1)
...
def main():
    ...
    print("CrossyLearn Agent - Modern Architecture w/ Baseline Agent")
    print("---------------------------------------------------------")
    ...
    # Timer for penalty zone event printing
    penalty_timer = 0
    ...
    while True:
        ...
        try:
            ...
            if frame is None:
                ...
                time.sleep(0.5)
                continue
            pass # Keep using the old vision_data
        ...
        if hwnd:
            left, top, right, bottom = win32gui.GetClientRect(hwnd)
            client_left, client_top = win32gui.ClientToScreen(hwnd, (left, top))
            window_geo = (client_left, client_top, 50) # (x, y, title_bar_height)
            ...
            if is_dead:
                ...
                if in_penalty_zone:
                    if penalty_timer == 0:
                        ...
                        penalty_timer = time.time() # Reset timer after printing
                else:
                    penalty_timer = 0
